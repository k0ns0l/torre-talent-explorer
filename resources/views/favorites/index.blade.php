...
<div class="space-y-6">
    ...
    @if($favorites->count() > 0)
        ...
        @if($profiles->count() > 0)
            <div>
                <h2 class="text-xl font-semibold text-gray-900 mb-4">Favorite Profiles ({{ $profiles->count() }})</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($profiles as $favorite)
                        <div class="profile-card">
                            <div class="flex items-start space-x-4">
                                @if($favorite->picture_url)
                                    <img src="{{ $favorite->picture_url }}" alt="Profile Picture" class="w-16 h-16 rounded-full object-cover">
                                @else
                                    <div class="w-16 h-16 rounded-full bg-gray-300 flex items-center justify-center">
                                        <span class="text-gray-600 text-xl">{{ substr($favorite->name, 0, 1) }}</span>
                                    </div>
                                @endif
                                
                                <div class="flex-1">
                                    <h3 class="font-semibold text-gray-900">{{ $favorite->name }}</h3>
                                    @if($favorite->professional_headline)
                                        <p class="text-sm text-gray-600 mt-1">{{ $favorite->professional_headline }}</p>
                                    @endif
                                    @if($favorite->location)
                                        <p class="text-xs text-gray-500 mt-2">📍 {{ $favorite->location }}</p>
                                    @endif
                                    <p class="text-xs text-blue-600 mt-1">@{{ $favorite->username }}</p>
                                </div>
                            </div>
                            
                            <div class="mt-4 pt-4 border-t border-gray-100 flex justify-between">
                                <a href="{{ route('talent.profile', $favorite->username) }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                    View Profile →
                                </a>
                                <button onclick="removeFavorite('{{ $favorite->username }}')" class="text-red-600 hover:text-red-800 text-sm">
                                    Remove
                                </button>
                            </div>
                        </div>
    ...
    @else
        <div class="text-center py-12">
            ...
            <p class="text-gray-600 mb-6">Start searching for profiles and jobs to add them to your favorites!</p>
        </div>
    @endif
</div>
